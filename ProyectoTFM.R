...
# data_clinical_sample.txt     # Información sobre el tipo de muestra
...
## Ajustamos para que solo diga I, II, III, IV
...
##Gráfica top 15 pero en procentaje
...
# 2. Clasificar en la tabla clínica
clinical_final <- clinical_final %>%
...
# Usamos el tiempo (OS_MONTHS) y el evento numérico (OS_EVENT)
...
# 4. Busca el "p-value" en la consola
surv_test <- survdiff(Surv(OS_MONTHS, OS_EVENT) ~ Grupo_Estudio, data = clinical_final)
print(surv_test)

### Diagrama de Venn ###
# 1. Obtenemos los IDs que sobrevivieron al filtro
ids_finales <- clinical_final$PATIENT_ID

# 2. Redefinimos los grupos usando SOLO esos IDs
...
# 2. Creamos el factor de grupo (importante el orden de los niveles)
## Ponemos KRAS primero para que sea el "control" y el resultado nos diga qué hace EGFR
grupo <- factor(clinical_final$Grupo_Estudio[pacientes_interes], 
                levels = c("KRAS_mut", "EGFR_mut"))

# 3. Preparamos la anotación 
library(org.Hs.eg.db)
...
## Forzamos a que 'genes_info' siga el MISMO orden que 'expression_final'
...
##. Seleccionamos los 30 genes más significativos del DEA
top_30_ids <- rownames(res_dea)[1:30]
top_30_symbols <- res_dea$SYMBOL[1:30]

##. Extraemos los datos de expresión (log_tpm) de esos genes y esos pacientes
matriz_hp <- log_tpm_subset[top_30_ids, ]
rownames(matriz_hp) <- top_30_symbols

##. Creamos una barra de color para identificar los grupos
df_col <- data.frame(Mutacion = grupo_dea)
rownames(df_col) <- colnames(matriz_hp)

##. Dibujamos el Heatmap
pheatmap(matriz_hp, 
         scale = "row",           # Normaliza por filas para ver contrastes
         annotation_col = df_col, # Añade la barrita de EGFR vs KRAS
         show_colnames = FALSE,   # Quitamos nombres de pacientes (son muchos)
...
##Creamos una función que testea la supervivencia gen por gen
# Usamos log_tpm para que la escala sea estadísticamente correcta
...
##. Unimos todo en una tabla y filtramos por P-valor < 0.05
...
## Ordenamos por p-valor para ver los más potentes arriba
...
# 2. Extraemos los datos de los 4 genes ganadores
genes_ids <- c("115416", "5723", "55065", "100506810")
coefs <- c(0.4446, 0.2028, -0.1932, -0.2740) 
names(coefs) <- genes_ids

# 3. Calculamos el Score sumando (Expresión * Coeficiente)
...
## a. Calculamos el modelo de Cox para los grupos
...
## ¿Qué predice mejor la muerte? ¿El Grupo_Estudio (mutaciones) o la Firma_Riesgo?
fit_comparativo <- coxph(Surv(OS_MONTHS, OS_EVENT) ~ Grupo_Estudio + Risk_Score_Z, 
                         data = clinical_final)
summary(fit_comparativo)

# 7. Factores de confusión: Firma, Edad y Estadio
...
# 8. ¿Varía el riesgo con el Estadio? (Si el p-valor es > 0.05, no hay relación)
...
# 9. ¿Varía el riesgo con la Edad? (Si el p-valor es > 0.05, no hay relación)
# Filtramos para el análisis de correlación
data_cor <- clinical_final[!is.na(clinical_final$AGE) & !is.na(clinical_final$Risk_Score), ]
# Ahora haces tu correlación y gráfico solo con los 481 (o los que queden limpios)
...
####### MARCADORES MICROAMBIENTE TUMORAL (TME) #############
# 1. Buscamos los IDs de los marcadores del microambiente
genes_tme_info <- AnnotationDbi::select(org.Hs.eg.db, 
                                        keys = c("CD274", "ACTA2", "CD163"), 
                                        columns = c("ENTREZID", "SYMBOL"), 
                                        keytype = "SYMBOL")

# 2. Extraemos la expresión de log_tpm (usando los ENTREZID)
...
# 3. Unimos estos nuevos datos a tu tabla clinical_final
...
ggplot(clinical_final, aes(x = Risk_Score, y = CD274)) +
  ...
  # Añadimos el texto estadístico dentro del gráfico
...
n_folds <- 10
# 1. Creamos los grupos de forma aleatoria
...
# Gráfico para ver la estabilidad
...
calcular_nb <- function(modelo, datos, umbral) {
  ## Predecimos la probabilidad de fallecer a los 3 años (36 meses)
